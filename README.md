...
export PATH=/usr/local/bin:$PATHをUser/ユーザー名/.bash_profileに書き込んでいます。
bashの設定を再読み込み
```$ source ~/.bash_profile```

### wget
WSLで使えます
```wget -P [savedir] [http://***]``` // -P 保存先指定
...
[VSCodeのオススメ拡張機能 24 選](https://qiita.com/sensuikan1973/items/74cf5383c02dbcd82234)
...
[インテリセンスの設定-->setting.jsonを編集](https://qiita.com/kusanoiskuzuno/items/fc6a32ef32dd9500f746)
外部モジュールのパス指定／インテリセンスエンジン（jedi）をFalseにする
...
※WSL2は正式リリース前なので、導入するとWindowsが不安定になる可能性があります。
...
[右クリックの重いメニューを調べるにはCCleanerを使用する](https://qiita.com/Q11Q/items/9fd5083ae71e32a577cd)
...
セキュアブート：無効化  
RufusでLiveUSB作成
- [構築手順](https://www.pc-koubou.jp/magazine/35542)  
Windows側で事前にパーティション操作はしないこと！
- [最初にやること](https://sicklylife.jp/ubuntu/1804/settings.html#gnome-initial-setup)  
ディレクトリを英語表記にする  
...
* path
[パスの書き方色々]
(https://qiita.com/ymdymd/items/d758110d429f72bc10fb)
...
言語毎にツールがある。pythonは、pep8, pylint, flake8 など。
...
- WSL
※wslランチャー：コマンドプロンプトでwslと入力

- vscode (Remote WSL)
 **※WSL/VirtualBoxのlinux環境ではGPUサポートしてない!**
 ※CPU利用のみのlinux実行環境のアプリ開発をする場合にあると便利
...
1.. vim ~/.bashrc
2.. python on Windows 側のaliasコメント反映
3.. source ~/.bashrc
 **※WSLの環境変数をexportで設定しても python.exe はWindowsの環境変数を参照するため、この方法の使い道はあまりない。**
...
実行->構成の追加-> launch.jsonが開く  
そこに"args":["--name", "hoge"],のように引数入力  
オプション付き引数を複数指定する場合、一行ずつargs:を追加  
...
ダウンロードした.debファイルは、/var/cache/apt/archives/にキャッシュされています。このキャッシュをクリアするには、以下のコマンドを実行します。
```sudo apt clean```
